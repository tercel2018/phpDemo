...
<template>
  <div class="wrapper">
    <!-- 头部 -->
    <header-and-nav title="财务中心" v-model="navOpen">
      <side-nav-cell icon="icon-qianbao" title="钱包明细" @on-click="toWalletDetail"></side-nav-cell>
      <side-nav-cell v-if="user_role==1" icon="icon-dengji" title="奖励金明细" @on-click="toBonusDetail"></side-nav-cell>
      <side-nav-cell v-if="user_role==11" icon="icon-dengji" title="佣金明细" @on-click="toCommisionDetail"></side-nav-cell>
    </header-and-nav>
    <!-- 导航 -->
...
export default {
  components: { Tab, TabItem, LoadMore, noData, Datetime, Group, XButton, headerAndNav, sideNavCell },
  data() {
    return {
      // 侧边导航隐藏
      navOpen: false,
  ...
  methods: {
    ...
    toWalletDetail() {
      this.navOpen = false
      this.$router.push('/main/finance_walletDetail')
    },
    // 跳转到奖励金明细
    toBonusDetail() {
      this.navOpen = false
      this.$router.push('/main/finance_bonusDetail')
    },
    // 跳转到佣金明细
    toCommisionDetail() {
      this.navOpen = false
      this.$router.push('/main/finance_commisionDetail')
    },
    // 点击导航
    filterFun(id) {
      let field = ''
    ...
    getAccountLogList(field = '') {
      ...
      field += '&page_size=' + this.pageSize + '&page=' + this.currPage + '&log_type=0' + '&add_time_start=' + this.start_time + '&add_time_end=' + this.end_time
